...
### 2.0 多平台与自定义列表 (Multi-Platform & Custom Lists) [New]
- **自定义列表 (Custom Lists)**：除了按平台分类的系统标签页外，现支持创建自定义列表。您可以将 X、微博、雪球等不同平台的用户混合添加同一个自定义列表中，实现跨平台的内容聚合。
- **普通网页链接支持 (Generic Web Links)** [New]：支持添加任意网页链接（如 `https://news.ycombinator.com`）。系统会自动识别域名格式，并在“获取”时自动在新标签页打开这些链接。注意：普通链接仅能添加到自定义列表中。
...
2.  **添加关注**：
    *   **平台用户**：在顶部输入框输入 X 用户名（例如 `elonmusk`）、微博昵称（例如 `央视新闻`）、雪球用户（例如 `但斌`）、小红书链接或 Substack 链接。
    *   **普通链接**：输入任意网址（如 `github.com`），系统会提示您将其添加到自定义列表中。
    *   点击 `Add` 按钮完成添加。
3.  **列表管理**：
    *   点击列表标题旁的 **更多 (⋮)** 按钮，可进行 **新建列表**、**导入** 或 **导出** 操作。
    *   悬停在自定义列表标题上，点击 **✎** 重命名或 **×** 删除列表。
4.  **调整视图**：
    *   在下拉菜单中切换不同列表（All / X / Custom Lists...）。
    *   在列表中勾选您想查看的用户/链接。
    *   在底部设置栏选择时间范围（默认 24h）。
5.  **获取内容**：点击底部的 **"Fetch"** 按钮：
    - **X/雪球**：自动抓取并聚合显示内容。
    - **其他平台/网页**：自动在后台打开选中博主的主页或网页链接。
6.  **设置与刷新**：顶部导航栏提供设置按钮（齿轮图标）和刷新按钮。点击设置按钮可切换 Feed 视图和列表管理视图。
7.  **切换语言**：点击顶部的 "EN/中" 按钮即可在英语和中文之间快速切换。
...
---
*Document Version 1.6 | Last Updated: 2026-01-29*
