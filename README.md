...
- **微博 (Weibo) 快速访问**：由于微博平台的限制，目前提供“一键直达”功能。在侧边栏中管理微博关注列表，点击 Fetch 时会自动在后台为您批量打开选中博主的个人主页，方便快速阅览。
- **分页管理 (Tabbed Interface)**：通过顶部的标签页（X / Weibo / Xueqiu）分类管理不同平台的用户，界面更加清爽有序。
- **别名系统 (Alias System)**：支持为用户设置备注名（Alias）。鼠标悬停在用户名上点击铅笔图标即可修改，特别适合记忆雪球等平台的数字 ID。
...
2.  **添加关注**：在顶部输入框输入 X 用户名（例如 `elonmusk`）、微博昵称（例如 `央视新闻`）或雪球用户（例如 `但斌`），点击 `Add` 按钮。
3.  **调整视图**：
    - 在顶部标签页切换平台（X / Weibo / Xueqiu）。
...
---
*Document Version 1.1 | Last Updated: 2026-01-20*
